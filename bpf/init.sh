...
function setup_proxy_rules()
{
	...
	if [ "$IP4_HOST" != "<nil>" ]; then
		...
		ip route replace table $TO_PROXY_RT_TABLE local 0.0.0.0/0 dev lo proto kernel
		...
		if [ "$ENDPOINT_ROUTES" = "true" ]; then
			...
			ip route replace table $PROXY_RT_TABLE $IP4_HOST/32 dev $HOST_DEV1 proto kernel
			ip route replace table $PROXY_RT_TABLE default via $IP4_HOST proto kernel
		fi
	else
	...
	if [ "$IP6_HOST" != "<nil>" ]; then
		...
		if [ -n "$IP6_LLADDR" ]; then
			# Traffic to the host proxy is local
			ip -6 route replace table $TO_PROXY_RT_TABLE local ::/0 dev lo proto kernel
			# Traffic from ingress proxy goes to Cilium address space via the cilium host device
			if [ "$ENDPOINT_ROUTES" = "true" ]; then
				...
				ip -6 route replace table $PROXY_RT_TABLE ${IP6_LLADDR}/128 dev $HOST_DEV1 proto kernel
				ip -6 route replace table $PROXY_RT_TABLE default via $IP6_LLADDR dev $HOST_DEV1 proto kernel
			fi
		fi
	else
